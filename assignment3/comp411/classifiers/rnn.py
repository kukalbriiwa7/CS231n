...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        #print("Grdient of W_vocab before clipping:")
        #print(grads['W_vocab'])
        ...
        if self.gclip != 0:
            for grad in grads:
                grad_norm = np.sum(grads[grad] * grads[grad])
                if grad_norm > self.gclip:
                    self.clip_grad_norm(grads, self.gclip)
                    break
        
        """
        if self.gclip != 0:
            if np.linalg.norm(grads['W_vocab']) > self.gclip :
                grads['W_vocab'] = self.clip_grad_norm(grads['W_vocab'],self.gclip)
            elif np.linalg.norm(grads['b_vocab']) > self.gclip :
                grads['b_vocab'] = self.clip_grad_norm(grads['b_vocab'],self.gclip)                
            elif np.linalg.norm(grads['Wx']) > self.gclip : 
                grads['Wx'] = self.clip_grad_norm(grads['Wx'],self.gclip)
            elif np.linalg.norm(grads['Wh']) > self.gclip :
                grads['Wh'] = self.clip_grad_norm(grads['Wh'],self.gclip)
            elif np.linalg.norm(grads['b']) > self.gclip : 
                grads['b'] = self.clip_grad_norm(grads['b'],self.gclip)
            elif np.linalg.norm(grads['W_embed']) > self.gclip :
                grads['W_embed'] = self.clip_grad_norm(grads['W_embed'],self.gclip)
        """                                                 
    ...
    def clip_grad_norm(self, grads, gclip):
        ...
        for grad in grads:
            grad_norm = np.sum(grads[grad] * grads[grad])
            if grad_norm > gclip:
                grads[grad] *= (gclip / grad_norm)


        clipped_grads = grads
        #clipped_grads = gclip * grads/np.linalg.norm(grads)
        #print("Gradients Shape is:")
        #print(grads)
    ...
    def sample_randomly(self, features, max_length=30):
        ...
        h_in, _ = affine_forward(features, W_proj, b_proj)
        word = self._start * np.ones((N), dtype=np.int32)
        softmax_lmbdfcn = lambda x : np.exp(x) / np.sum(np.exp(x), axis=0)

        for i in range(max_length):
            emb, _ = word_embedding_forward(word, W_embed)
            h_in, _ = rnn_step_forward(emb, h_in, Wx, Wh, b)
            scores, _ = affine_forward(h_in, W_vocab, b_vocab)
            pdfcn = softmax_lmbdfcn(scores[0])
            random_word = np.array(np.where(scores[0] == np.random.choice(scores[0], p=pdfcn)))
            captions[:,i] = random_word
            word = random_word
