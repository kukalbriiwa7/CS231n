...
class ThreeLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        loss = -np.sum(np.log(loss)) / num_train + reg * (np.sum(W1 * W1) + np.sum(W2 * W2) + np.sum(W3 * W3))
        ...
        hidden2 = s.dot(W3.T)
        hidden2[H2 == 0] = 0
        dW2 = H1.T.dot(hidden2)
        db2 = np.sum(hidden2, axis=0)
        hidden1 = hidden2.dot(W2.T)
        hidden1[H1 == 0] = 0
        dW1 = X.T.dot(hidden1)
        db1 = np.sum(hidden1,axis = 0)
    ...
    def predict(self, X):
        ...
        #output_of_first_layer = np.maximum(0,X.dot(self.params['W1']) + self.params['b1'])
        #output_of_second_layer = np.maximum(0,output_of_first_layer.dot(self.params['W2']) + self.params['b2'])
        #output_of_third_layer
