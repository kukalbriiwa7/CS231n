...
class KNearestNeighbor(object):
    ...
    def compute_L1_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            dists[i,:] = np.sum(abs(X[i] - self.X_train), axis = 1)
    ...
    def compute_L1_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        # d = np.dot(X.sum(axis=1).reshape(-1,1),self.X_train.sum(axis=1).reshape(-1,1).T)
        
        
        #dists = cdist(X, self.X_train, metric='cityblock')
        
        ## THIS IS WAY TOO SLOW BUT I HAVE IMPLEMENTED IT ANYWAY ##
        diff = np.reshape(X, (num_test,1,3072)) - self.X_train
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            # closest_y array has the labels of k nearest pictures in the traning set. 
            # When we give it to the bincount function,the value of the most freqent element will be 
            # the index of the largest number in the output of bincount function. So by finding the index of the maximum
            # element in ourput of bincount we found the value of the most frequent entry in closest_y.
